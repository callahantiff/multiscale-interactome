...
class DiffusionProfiles:
    def __init__(self, alpha, max_iter, tol, weights, num_cores, save_load_file_path):
        ...
        self.initial_M = None
        self.drug_or_indication2diffusion_profile = None
    ...
    def convert_M_to_make_all_drugs_indications_sinks_except_selected(self, msi, selected_drugs_and_indications):
        ...
        for drug_or_indication in selected_drugs_and_indications:
            ...
            idxs_to_remove_in_edges = [msi.node2idx[protein] for protein in
                                       proteins_pointing_to_selected_drug_or_indication]
    ...
    @staticmethod
    def get_personalization_dictionary(nodes_to_start_from, nodelist):
    ...
    def power_iteration(self, M, S, nodelist, per_dict):
        ...
        if missing:
            missing_message = 'Personalization dictionary must have a value for every node. Missing nodes %s'
            raise nx.NetworkXError(missing_message % missing)
    ...
    @staticmethod
    def batch_list(list_, batch_size=None, num_cores=None):
    ...
    def calculate_diffusion_profiles(self, msi):
        ...
        for computation_list_batch in computation_list_batches:
            ...
            proc = multiprocessing.Process(target=self.calculate_diffusion_profile_batch,
                                           args=(msi, computation_list_batch))
            procs.append(proc)
            proc.start()
    ...
    def load_diffusion_profiles(self, drugs_and_indications):
        ...
        for drug_or_indication in drugs_and_indications:
            file_path = os.path.join(self.save_load_file_path,
                                     self.clean_file_name(drug_or_indication) + "_p_visit_array.npy")
